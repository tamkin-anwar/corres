...
2. **Recipient-controlled, not sender-controlled.** The Screener (2026-09-21): a genuinely new sender (one with no prior mail already in the account) is held out of Brief/Needs You/Waiting/Mail until reviewed, approved once and every future message from them flows normally, blocked once and they're silently gone, no notification, ever again, matching HEY's own model. Never quarantines an existing mailbox: every sender already present at an account's first sync is auto-approved as an established relationship; only a sender arriving after that baseline gets screened. Still findable via explicit search either way, same as a snoozed thread, never truly hidden.
...
4. Full and incremental sync, inbox and sent threads, reconciliation, retries, deletion, and stale-history recovery. Incremental sync done (2026-09-21): a `historyId` cursor per account, capped full listing on first sync or an expired cursor, real sender/subject/body. Sent-thread sync also done, same day: a thread living only in Sent now syncs too, not only Inbox. Cursor/upsert ordering reviewed, not changed: the cursor is deliberately written only after the local write succeeds, since that ordering already avoids real data loss (see ADR 005); full cross-store atomicity was consciously not pursued for a failure mode that's already benign. Still open: reconciliation/retry beyond a single fetch attempt; a reply to a Corres-composed new message does not yet update that thread's content when it arrives (see ADR 005's "known limitation").
5. Durable drafts, safe compose/reply, attachments, and explicit sending. Never silently retry an ambiguous send. Real sending done (2026-09-21) for replying/replying all/forwarding within an existing synced thread and, same day, for a brand-new from-scratch compose too: a proper RFC 5322 message via `users.messages.send`, correct Gmail threading for replies (`threadId` + In-Reply-To/References + matching Subject), and an instant-feeling optimistic send with an undo window and a Retry/Discard path on real failure (see ADR 005, `OutboxService`). The outbox itself is durable as of the same day: a queued or failed send is persisted immediately, and survives the app being force-quit mid-undo-window (see ADR 005, `resumeAfterRelaunch`). Still open: attachments; bounded retries with jitter; per-account serialization.
