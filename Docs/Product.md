...
4. Full and incremental sync, inbox and sent threads, reconciliation, retries, deletion, and stale-history recovery. Incremental sync done (2026-09-21): a `historyId` cursor per account, capped full listing on first sync or an expired cursor, real sender/subject/body, still one-way (no writes back to Gmail). Still open: sent-thread sync, reconciliation/retry beyond a single fetch attempt, and cursor advancement is not yet atomic with the local write it follows. See ADR 005.
