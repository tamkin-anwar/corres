...
- **Confirmed but not yet fixed at the time** (tracked for later, listed honestly rather than silently dropped): triage's missing confidence threshold (Batch A, fixed below), sync silently dropping individual failed message fetches while still advancing its cursor (Batch B, fixed below), a silent SwiftData-to-in-memory storage fallback (Batch B, fixed below), the push relay's total lack of request authentication (Batch C, fixed below for `/pubsub`), and a test suite that hasn't grown alongside two weeks of real feature work.

## Batch A: triage confidence threshold (September 24, 2026)

Researched what Foundation Models' structured generation can actually expose before designing this — no token-level log-probabilities, so a self-reported discrete confidence level (`low`/`medium`/`high`) is the honest option, not a continuous score dressed up to look more precise than it is.

- `TriageAssessment.Confidence` (new `@Generable` enum) is a real field now, filled in by the model every assessment; `SemanticTriageService.triage` only lets a downgrade to `.quiet` through at `medium` or `high` confidence.
- Verified with `xcodebuild` (`BUILD SUCCEEDED`) and `swift test` (46/46 — this path has no unit test of its own, since exercising it for real needs the actual on-device model, not something a deterministic test can stand in for).
- **Not yet verified on-device, and this is the one that matters most here**: whether the model's self-reported `low`/`medium`/`high` is actually well-calibrated against real inbox mail — i.e., whether things it calls "high confidence, no reply needed" really don't need one — can only be judged by running it against a real inbox and checking, not by reading the prompt. The `medium`+ bar is a reasonable starting point, not a tuned one.

## Batch B: sync fetch failures and the silent storage fallback (September 24, 2026)

Researched the tradeoffs before picking a design: Gmail's history cursor is one whole-mailbox counter that can't be partially advanced, so neither "block the whole sync on one flaky message" nor "retry harder inline and hope" alone was enough — needed both a bounded per-message retry and a persisted, cursor-independent retry queue for whatever's still missing after that.

- `GmailAPIClient.fetchMessages` now retries each individual message fetch up to 3 times before giving up; `SyncResult.failedMessageIDs` reports whatever's still missing instead of silently dropping it. `GmailSyncService` persists that set per account and retries it by id directly on every later sync, independent of the cursor, capped at 200 tracked ids so a genuinely-gone message can't make this grow forever.
- `AppDelegate.makeModelContainer`'s in-memory fallback (for a corrupt/incompatible on-disk SwiftData store) now surfaces through `MailStore.errorMessage` — the same "Could not save" alert every other save failure already uses — instead of failing silently.
- Verified with `xcodebuild` (`BUILD SUCCEEDED`) and `swift test` (46/46, unaffected).
- **Neither of these has an automated test, and both genuinely need one before they can be called done**: the missed-ids retry needs a real flaky Gmail connection (or a way to simulate one) to confirm messages actually recover instead of just compiling; the storage fallback needs an actually-corrupt on-disk store to trigger at all. Both are real-device/real-failure-condition concerns that a clean build can't speak to.

## Batch C: push relay authentication (September 24, 2026)

Researched the real constraint before designing anything: one Cloud Function/Cloud Run service has one IAM invoker policy for its whole URL, so `/pubsub` couldn't be locked down without either also locking down `/register`/`/unregister` (which must stay public) or moving `/pubsub` to its own function. Chose the split, then actually deployed and verified it against the real `corres-509320` project — this is the one batch that isn't just a code change, and "the code looks right" isn't what verification means here.

- `corres-push-relay-pubsub` (new function) deployed without `--allow-unauthenticated`; `gmail-push-invoker` (new service account) is the only identity granted `roles/run.invoker` on it; Pub/Sub's own service agent granted `roles/iam.serviceAccountTokenCreator` on that service account; `gmail-push-sub`'s push config updated with `--push-auth-service-account` pointing at it.
- `corres-push-relay` (existing, still public) redeployed with real input validation (device-token/email format, a 50-account-per-device cap) and the old `/pubsub` route removed entirely.
- **Actually verified end-to-end with real `curl` calls against the live deployment, not assumed from the code**:
  - `POST corres-push-relay-pubsub` with no auth → real `403` from Google's own front end (confirms the IAM binding is actually enforced, not just configured).
  - `POST corres-push-relay/pubsub` (the old path) → `404` (confirms the route is genuinely gone, not just newly unauthenticated-and-still-present).
  - `POST corres-push-relay/register` with a malformed email/device-token → `400` (confirms the new validation deployed); with well-formed values → `204`, then `unregister` on the same device token → `204` (confirms the public routes still work for the app).
  - `gcloud pubsub subscriptions describe gmail-push-sub` confirms `pushConfig.oidcToken.serviceAccountEmail` and `pushConfig.pushEndpoint` both point at the right places.
- **What this can't verify from here**: a real Gmail mailbox change actually flowing through Pub/Sub → the authenticated function → a real APNs push landing on a real device. The IAM/OIDC wiring is confirmed correct and matches Google's own documented pattern for authenticated push subscriptions, but the full live path needs a real connected account and a real device to watch it land.
- **Deliberately not fixed, and said so directly rather than silently**: `/register`/`/unregister` still have no way to verify a caller is a genuine Corres install. The real fix is Apple's App Attest framework — scoped out of this pass, documented as an open gap in `Server/push-relay/README.md`.
