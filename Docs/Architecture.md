...
Still planned: commit applied changes and cursor advancement atomically (today, the cursor is written after `upsert` completes but the two are not transactional together); a durable outbox for retries if either step fails mid-sync.
